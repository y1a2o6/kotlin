...
class W2(val f: E1) {
//    constructor(f: Int.() -> Int) : this(fun Int.(String): Int = f()) // ni-(EXPECTED_PARAMETERS_NUMBER_MISMATCH)
}
...
class X(val f: Int.(String) -> Unit)

val x1 = X { this + it.length }
val x2 = X { s -> this + s.length }
val x3 = X { i, s -> i + s.length }
val x4 = X { i, s: String -> i + s.length }
val x5 = X { i: Int, s -> i + s.length }
val x6 = X { i: Int, s: String -> i + s.length }

class Y(val f: (Int, String) -> Unit)

val y1 = Y(id<Int.(String) -> Unit>(){ this + it.length })
val y2 = Y(id<Int.(String) -> Unit>(){ s -> this + s.length })
val fy3: Int.(String) -> Unit = { s -> this + s.length }
val y3 = Y(fy3)
val y4 = Y { i, s -> i + s.length }
