...
//        }
    ...
    private fun findReferenceDataToRestore(
        ...
        return findReferencesToRestore(file, blockStart, sourceFileBasedReferences.map { pair -> pair.second }.toTypedArray())
    }
    ...
    private fun findReferences(
    ...
    ): List<Pair<KtElement, KotlinReferenceData>> {
        ...
        elementsInRange.forEach { element ->
            element.forEachDescendantOfType<KtElement>(canGoInside = {
                ...
                if (descriptors.isEmpty()) return@forEachDescendantOfType
                ...
                for (descriptor in descriptors) {
                    ...
                    if (declaration != null && declaration.isInCopiedArea(file, textBlocks) ||
                    ...
                    val kind = KotlinReferenceData.Kind.fromDescriptor(descriptor) ?: continue
                    val isQualifiable = KotlinReferenceData.isQualifiable(ktElement, descriptor)
                    val relativeStart = ktElement.range.start - data.startOffset
                    val relativeEnd = ktElement.range.end - data.startOffset
                    resolvedReferences
                        .add(
                            ktElement to
                                    KotlinReferenceData(relativeStart, relativeEnd, fqName, isQualifiable, kind)
                        )
                }
            }
        }
    ...
    private fun PsiElement.isInCopiedArea(fileCopiedFrom: KtFile, textBlocks: List<TextBlock>): Boolean {
        if (containingFile != fileCopiedFrom) return false
        val any = textBlocks.map { TextRange(it.startOffset, it.endOffset) }.any { this.range in it }
        return any
    }
